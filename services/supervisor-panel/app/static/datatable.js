...
(function () {
  ...
  var RENDERERS = {
    ...
    code: function (d) {
      if (d == null || d === '' || d === '—') return '<span style="opacity:.4;">—</span>';
    ...
    statusProgress: function (d, type, row) {
      ...
      return '<div class="job-status-cell">'
        + '<span class="badge ' + cls + '">' + esc(d) + '</span>'
    ...
    truncate: function (d, type) {
      ...
      return '<span title="' + esc(s) + '">' + esc(s.substring(0, 80)) + '…</span>';
    },
    ...
    jobOutput: function (d, type, row) {
      ...
      if (d === 'completed' && row.version_name) {
        return '<span style="color:var(--green);font-size:.85rem;">'
          + '<i class="fa-solid fa-check"></i> ' + esc(row.version_name) + '</span>';
      }
      if (d === 'failed' && row.error_message) {
        var msg = String(row.error_message);
        return '<span style="color:var(--red);font-size:.78rem;" title="' + esc(msg) + '">'
          + '<i class="fa-solid fa-triangle-exclamation"></i> '
          + esc(msg.substring(0, 60)) + (msg.length > 60 ? '…' : '') + '</span>';
      }
    ...
    sessionId: function (d, type, row) {
      if (type !== 'display') return d;
      var dot = row.status === 'active'
        ? '<span class="live-dot" style="margin-right:6px;"></span>'
        : '';
      return '<div class="flex-center">'
        + dot
        + '<a href="/sessions/' + row.id + '" class="td-link td-mono">' + esc(d) + '</a>'
        + '</div>';
    },
    ...
    hardDecline: function (d, type) {
      ...
      if (n === 1) return '<span class="badge badge-risk-medium">' + n + '</span>';
    ...
    correctionLink: function (d, type, row) {
      ...
      return '<a href="/sessions/' + row.session_id + '/turns/' + row.turn_id
    ...
    approveReject: function (d, type, row) {
      ...
      return '<div style="display:flex;gap:4px;">'
        ...
        +   'data-url="/training-candidates/' + row.id + '/approve" ' + appDis + '>Approve</button>'
        + '<button class="btn btn-sm btn-danger dt-action-btn" '
        +   'data-url="/training-candidates/' + row.id + '/reject" ' + rejDis + '>Reject</button>'
        + '</div>';
    },
    ...
    sessionLink: function (d, type, row) {
      if (type !== 'display') return '';
      return '<a href="/sessions/' + row.id + '" class="btn btn-sm btn-outline">'
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('table[data-dt-url]').forEach(function (table) {
      ...
      var pollVals   = (table.getAttribute('data-dt-poll-values') || 'running,pending').split(',');
