...
class Session(Base):
    ...
    voice_events: Mapped[list["VoiceEvent"]] = relationship("VoiceEvent", back_populates="session")
...
class VoiceEvent(Base):
    """Durable M8 voice-runtime audit event.

    event_id is generated by the runtime and unique so reconnect/retry paths can
    persist events idempotently without duplicating the supervisor timeline.
    """
    ...
    session: Mapped["Session"] = relationship("Session", back_populates="voice_events")
    turn: Mapped["Turn | None"] = relationship("Turn")
