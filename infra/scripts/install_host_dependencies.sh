...
if command -v sysctl >/dev/null 2>&1; then
  cat >/etc/sysctl.d/99-fine-tuned-agent-livekit.conf <<'EOF'
...
mkdir -p /opt/fine-tuned-agent/{repo,models,data,backups,logs}
echo "Klasörler oluşturuldu: /opt/fine-tuned-agent/"
