# Anrufblocker Platform — Teknik Referans Dökümantasyonu
...
|----------|---------|---------|
| `PROJECT_NAME` | `anrufblocker-platform` | Log prefix'i, metadata |
...
| `VLLM_MODEL_NAME` | `anrufblocker-v14` | vLLM'e atılan `model` parametresi. vllm-server'daki `--served-model-name` ile eşleşmeli |
...
| `MODEL_ACTIVE_VERSION` | `anrufblocker-v14` | Model registry'de hangi versiyonun "active" sayıldığını belirler |
| `MODEL_DIR` | `/models` | Container içindeki model dizini |
| `MODEL_MERGED_PATH` | `/models/merged/anrufblocker-v14` | Aktif merged modelin tam yolu. vllm-server bu dizini `/model` olarak mount eder |
...
| `LIVEKIT_AGENT_NAME` | `anrufblocker-voice` | Worker'ın register olduğu agent adı. Token'daki `RoomAgentDispatch.agent_name` ile eşleşmeli |
...
```bash
psql -h 127.0.0.1 -p 5432 -U anrufblocker -d anrufblocker
